...
def top_movers():
    ...
    for gainer in gainers:
        new.append([gainer["symbol"] , gainer["name"] , gainer["changesPercentage"] , gainer["exchange"]])
    gainers[:] = new

    new = []
    for loser in losers:
        new.append([loser["symbol"] , loser["name"] , loser["changesPercentage"] , loser["exchange"]])
    losers[:] = new
...
def lesson_progress():
    ...
    with sqlite3.connect('main.db') as conn:
        cursor = conn.cursor()
        cursor.execute(f"SELECT chapter{chapter_number}, percentage_complete, contributions FROM main WHERE userid = ?", (userid,))
        row = cursor.fetchone()
        ...
        cursor.execute(f"UPDATE main SET chapter{chapter_number} = ?, percentage_complete = ?, contributions = ? WHERE userid = ?", 
...
def get_lesson_progress():
    ...
    with sqlite3.connect('main.db') as conn:
        ...
        cursor.execute("SELECT chapter1, chapter2, chapter3, chapter4, chapter5, chapter6, chapter7, chapter8, percentage_complete FROM main WHERE userid = ?", (userid,))
        row = cursor.fetchone()
    ...
    if not financials.empty and not balance_sheet.empty:
        ...
        for year in years:
            ...
            if 'Total Revenue' in financials.index and year in financials.columns:
                ...
                ps_ratio = info.get('currentPrice', 0) / revenue_per_share if revenue_per_share != 0 else 'N/A'
        ...
        current_metrics = {
            'Total_Cash': balance_sheet.loc['Cash And Cash Equivalents', balance_sheet.columns[0]] if 'Cash And Cash Equivalents' in balance_sheet.index and not balance_sheet.empty else 'N/A',
            'Total_Cash_Per_Share': info.get('totalCashPerShare', 'N/A'),
            'Revenue': financials.loc['Total Revenue', financials.columns[0]] if 'Total Revenue' in financials.index and not financials.empty else 'N/A',
            'Gross_Profit': financials.loc['Gross Profit', financials.columns[0]] if 'Gross Profit' in financials.index and not financials.empty else 'N/A',
            'Total_Debt': balance_sheet.loc['Total Debt', balance_sheet.columns[0]] if 'Total Debt' in balance_sheet.index and not balance_sheet.empty else 'N/A',
            'Total_Debt_to_Equity': info.get('debtToEquity', 'N/A'),
            'Operating_Cash_Flow': cashflow.loc['Operating Cash Flow', cashflow.columns[0]] if 'Operating Cash Flow' in cashflow.index and not cashflow.empty else 'N/A'
        }
